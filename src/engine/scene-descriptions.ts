...
export function generateSceneDescription(
  ...
  if (scene === "classroom") {
    ...
    if (period === "evening") return "放学后的教室，安安静静的。";
    if (period === "night") return "夜里的教室，黑漆漆一片。";
  }

  if (scene === "playground") {
    if (names.length >= 2) return `${names[0]}在跑步，${names[1]}在旗杆旁站着。`;
    if (names.length === 1) return `${names[0]}一个人在操场上。`;
    if (weather === "rainy") return "🌧 下雨了，操场上没有人。";
    return "操场空空的，风吹过旗杆。";
  }
  ...
  if (period === "night") return "🌙 夜深了，安安静静的。";

  return names.length > 0 ? `这里有${names.join("、")}。` : "";
}
